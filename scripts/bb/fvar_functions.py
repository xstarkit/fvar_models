...
def vary_parameters(variable_pars,N, cor, kT, frac):
    print("Varying parameters and calculating spectra...")
    Plot.device = "/xw"
    Plot("eemodel")

    gamma_offset=2+12.*cor
    gamma_par=4
    pl_flux_par=3
    kT_par=9
    bb_flux_par=8
    ...
    bb_flux=initial_pars[0][0]+np.log10(frac)
    # print(bb_flux)
    AllModels(1).setPars({kT_par:kT})
    ...
    for i in range(0,N):
        ...
        new_gamma=cor*pl_flux+gamma_offset
        # print(new_gamma)
        if new_gamma<-3:
